...
//		"1024 联合国日",	"1111 光棍节",	"1220 澳门回归纪念",
//		"1225 圣诞节"
//	};
	/**
	 * 国历节日 *表示放假日
	...
	private final static String[] lFtv = {
		"0101*春节","0115 元宵节",
		"0505*端午节","0707 七夕节",	"0815*中秋节","0909 重阳节",		
		"1208 腊八节" ,	"0100*除夕"
	};
	...
	private ArrayList 	isFesArray = new ArrayList<String>();
...
//		"0520 母亲节", "0716 合作节", "0730 被奴役国家周"
	...
	/**
	 * 判断今天是否是法定假日  （只能判断一年）
	 * @param strDate  五月一号数据格式“0501”
	 * @return true 是法定假日
	 */
	public boolean isFestival(String strDate) {
		if ( isFesArray.size() <= 0) {
			for (int i =0; i< isFes.length ; i++) {
				isFesArray.add(isFes[i]);
			}
		}
		if (isFesArray.size() > 0) {
			if (isFesArray.contains(strDate)){
				return true;
			} else {
				return false;
			}
